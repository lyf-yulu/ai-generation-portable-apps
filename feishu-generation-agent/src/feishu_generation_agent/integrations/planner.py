...
每个 prompt 必须严格按下面的模板骨架逐句展开，尖括号是要替换的槽位，其余固定句式原样保留、顺序不变：
「禁止勾勒边缘线，画面风格严格参考图一重新生成图片，<景别>，<时间与场景>，画面内容参考<人物如何自然设身处地融入场景>，<画面中人物的活动>，背景是<背景>。画面主次分明，<画风提示词>。画面主体控制在画布<宽>*<高>画面中央，光影自然，冷暖对比，光线暖柔，整体氛围<氛围>，整体画面高亮度高明度，画面风格严格参考图一生成图片，禁止勾勒边缘线，禁止勾勒边缘线，禁止拉伸图片，<时间>，禁止出现窗户」
槽位取值来自需求文档：景别取「内容描述」里的中景/近景/全景等；时间与场景取该编号的对应场景与时间；人物活动取内容描述里的动作与表情；画风提示词取「风格参考」一节的关键词；画布宽高写该需求小节的完整版尺寸。
「禁止勾勒边缘线」按模板出现三次，是刻意强调，不要合并或删减。
末尾的时间与「禁止出现窗户」按文档实际情况写：文档写白天就写白天，夜景需求写夜晚，不得一律套白天；场景本身就该有窗户时删掉「禁止出现窗户」这句。
...
（1）角色参考（「需求登场角色及角色状态」一节）：只用于人物外形与服装，明确不参考画风；prompt 里说明该角色沿用哪张。
