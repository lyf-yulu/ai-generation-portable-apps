...
title Nano Banana Launcher

set "APP_TITLE=Nano Banana"
set "APP_DIR=%~dp0"
cd /d "%APP_DIR%" || goto fail

if not exist "%APP_DIR%logs" mkdir "%APP_DIR%logs" >nul 2>nul
set "LOG_FILE=%APP_DIR%logs\nano_banana_windows.log"
set "START_LOG=%APP_DIR%logs\nano_banana_windows_startup.log"
...
set "PORT=%PORT%"
start "Nano Banana Server" /B "%PYTHON%" "%APP_DIR%app.py" >> "%LOG_FILE%" 2>&1
