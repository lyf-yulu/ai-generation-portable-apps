...
title Nano Banana Launcher - China Mirror

set "APP_DIR=%~dp0"
cd /d "%APP_DIR%"

echo.
echo Nano Banana Launcher - China Mirror
...
echo Log file: logs\nano_banana_windows.log
echo.

start "" "%URL%"

"%PYTHON%" "%APP_DIR%app.py" >> "%APP_DIR%logs\nano_banana_windows.log" 2>&1

echo.
echo Program exited. If the webpage cannot open, send logs\nano_banana_windows.log to the maker.
echo.
pause
