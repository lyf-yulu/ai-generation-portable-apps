...
`com.feishu-generation-agent` 的 launchd 服务守护。Portal 的主机、协议和
端口均由部署配置及当前访问环境决定，已有登录账号、角色和反向代理能力。
...
飞书只读探测已确认，内嵌电子表格 token 可以从 Block 30 取得。直接调用
Sheets API 会因权限不足返回 `99991672`，但现有服务端飞书应用已经可以通过
Drive 导出接口取得 XLSX。对“魂穿”文档的只读验证得到约 31.9 MB 的 XLSX，
其中包含 9 个唯一图片文件和 10 个图片锚点。因此首版采用 XLSX 导出作为主路径，
不要求额外开通 Sheets 权限；Sheets API 仅作为未来可选增强。
...
```mermaid
flowchart LR
    U["Portal 登录用户"] --> P["Portal（配置化访问地址）"]
...
1. 从 Block 30 的 token 中拆分 spreadsheet token 与 sheet ID；
2. 通过飞书 Drive 导出任务接口请求 XLSX，并在有限时间内轮询导出状态；
3. 下载 XLSX，使用 ZIP/XML 读取共享字符串、目标工作表、图片关系和锚点；
4. 将图片转换为现有 `MediaAsset`，将单元格文字和图片位置按工作表、行列锚点
   与文档顺序插回 `text_view`；
5. 如果未来服务端飞书应用具备 Sheets 只读权限，可以增加直接读取作为优化，
   但不得改变 XLSX 路径的行为和测试覆盖。
...
- Drive 导出接口返回权限错误：标记为阻断性读取问题，并显示需要检查文档访问
  权限，不继续生成一个看似完整的计划；
- 导出超时、XLSX 过大或目标工作表解析失败：显示工作表 ID 和失败原因；
- 单张图片解析失败：其余图片继续进入视觉理解，但审批页必须展示缺失素材；
- 不允许静默忽略整个内嵌表格。
...
原运行完成后，重跑必须从最后实际执行的完整批准计划开始重新审批，而不是回退到
规划器最初产生的草稿。恢复顺序如下：

1. 源 checkpoint 存在 `approved_plan` 时，校验并直接使用它；
2. 旧 checkpoint 缺少 `approved_plan` 时，用 `draft_plan` 与
   `approved_tasks` 经同一素材覆盖协调函数重建；
3. 将恢复出的完整计划同时写入新运行的 `draft_plan` 与 `task_plan`；
4. 保留其中的任务作为默认选中项，但清空批准决定、`approved_plan`、执行记录、
   产物和交付结果，使新运行停在重新审批；
5. clone 本身不得调用图片或视频生成供应商。

因此，用户上次审批中对中文提示词、参考素材、生成参数和排除理由的修改都会出现在
新审批页中；只有再次批准后才可能产生新的付费调用。
...
- 飞书 App ID、Secret、文档权限、DeepSeek Key 和生成供应商 Key 全部留在
  服务机后端；
...
- XLSX 导出权限缺失或解析失败：显示可执行的错误说明，不返回“成功但只有两张
  图”的计划；
...
- 普通无 Block 30 的文档继续沿用现有读取路径；含 Block 30 的文档通过当前已有
  权限走 XLSX 导出，导出不可用时明确阻断并提示权限或解析错误。
...
8. 主要规划字段无中文时触发修复；
9. 英文对白在中文主体提示词中得到保留；
10. Block 30 token 拆分、XLSX 导出轮询、文字与图片锚点提取；
11. 所有素材必须被引用或明确排除；
12. 用户只能读取和操作自己的运行；
13. 所有用户共享同一生产任务锁；
...
1. 在测试 Portal 和测试端口验证代理、前缀路径及身份头；
2. 用两个 Portal 测试账号验证提示词和运行隔离；
3. 直接访问 `8765`，确认仍为 Prime 模式；
4. 重读“魂穿”需求，核对普通图片、内嵌表格 9 个唯一图片文件、10 个图片锚点
   和素材覆盖统计；
5. 完整执行一个测试任务并确认结果写入共享结果表；
6. 确认当前没有运行中任务后重启一次 Agent，使后端代码生效；
7. 更新 Portal 配置并重启 Portal；
8. 从另一台客户端通过当前部署公布的 Portal 地址登录，完成扫描、领取、审批和
   查看结果的最终验收。
