...
+ [修复] 单节点修改提示词后重新生成：任务轮询遇到瞬时网络错误不再中止，会按退避间隔自动重试直到出结果；任务已永久缺失时清理保存的引用并明确报错，结果图无需切换页面回来才出现。
+ [新增] 画布节点支持独立放大缩小（25%–400%，步进 25%），选中节点显示缩放控件，内部控件与素材/生成结果节点的图片、视频预览随节点等比例缩放。
+ [新增] 生成结果节点按生成时的比例与尺寸档位计算真实大小（如 2K·16:9 → 3641×2048），并生成在模型节点右侧避免遮挡。
+ [修复] 画布容器改为不可滚动裁剪，避免控件获得焦点时浏览器滚动画布导致整体位移。

### AI 创作画布隔离验证

+ [新增] 基于受控 Portal 的图像、视频与人像资产视频验证链路，以及用户归属、任务结果代理和用量隔离。
+ [新增] 可复现发布包、Python-only 运行入口和隔离验收文档。
+ [说明] 未部署生产，未执行真实模型调用或生产进程比较；公开发布仍需另行书面批准。
