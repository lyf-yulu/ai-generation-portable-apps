...
export function ObjectLifecycleActions<T>({ objectIdentity, objectLabel, enabled, archivedAt = null, revision, historical = false, onEnable, onDisable, onArchive, onRestore, onDelete, onPurge, onChanged, onDeleted, onRefresh }: Props<T>) {
    ...
    return <div className="mt-4 border-t border-[#222e48] pt-4">
        <div className="flex flex-wrap gap-2">
            {!historical && (archivedAt ? <button type="button" disabled={busy} onClick={() => void perform(onRestore)} className="rounded border border-[#405076] px-3 py-1.5 text-sm text-[#99b3f0] disabled:opacity-50">恢复</button> : <>
                {enabled ? <button type="button" disabled={busy} onClick={() => void perform(onDisable)} className="rounded border border-[#405076] px-3 py-1.5 text-sm text-[#d0e8d6] disabled:opacity-50">停用</button> : <button type="button" disabled={busy} onClick={() => void perform(onEnable)} className="rounded border border-[#405076] px-3 py-1.5 text-sm text-[#99b3f0] disabled:opacity-50">启用</button>}
        ...
        {error?.code === "REVISION_CONFLICT" && <div role="alert" className="mt-3 flex flex-wrap gap-2 text-sm text-[#92400e]">配置已变化，请重新加载。{onRefresh && <button type="button" onClick={onRefresh} className="underline">重新加载</button>}</div>}
        {error?.code === "RESOURCE_REFERENCED" && <div role="alert" className="mt-3 text-sm text-[#92400e]">对象仍被引用：{Object.entries(error.references || {}).map(([key, count]) => `${referenceLabels[key as keyof typeof referenceLabels]} ${count}`).join("，") || "引用状态不可用"}。可保留审计记录并清理运行配置。</div>}
        {error && !["REVISION_CONFLICT", "RESOURCE_REFERENCED"].includes(error.code) && <p role="alert" className="mt-3 text-sm text-[#92400e]">操作未完成，请重试。</p>}
        {confirm && <div role="dialog" aria-modal="true" aria-labelledby="object-confirm-title" onKeyDown={(event) => { if (event.key === "Escape") { event.preventDefault(); close(); } }} className="fixed inset-0 z-50 flex items-center justify-center bg-black/75 p-4">
            <div className="w-full max-w-md rounded-xl border border-[#405076] bg-[#080b11] p-5 shadow-2xl">
                <h2 id="object-confirm-title" className="text-lg font-semibold text-[#172033]">{confirm === "delete" ? "确认删除" : "确认清理历史运行配置"}</h2>
                <p className="mt-2 text-sm text-[#acb4c6]">请输入 <strong className="text-[#172033]">{objectLabel}</strong>。该操作不会显示或保留任何部署凭据。</p>
                <label className="mt-4 block text-sm text-[#c9decf]">输入 {objectLabel} 确认{confirm === "delete" ? "删除" : "清理"}<input ref={confirmInput} aria-label={`输入 ${objectLabel} 确认${confirm === "delete" ? "删除" : "清理"}`} value={typed} onChange={(event) => setTyped(event.target.value)} className="mt-1 block w-full min-w-0 rounded border border-[#405076] bg-[#0c0f17] px-3 py-2 text-[#172033]" /></label>
                <div className="mt-4 flex justify-end gap-2"><button type="button" onClick={close} className="rounded border border-[#405076] px-3 py-2 text-sm text-[#c9decf]">取消</button><button type="button" disabled={busy || typed !== objectLabel} onClick={() => void destructive()} className="rounded bg-[#a94235] px-3 py-2 text-sm font-medium text-white disabled:opacity-40">{confirm === "delete" ? "确认删除" : "确认清理"}</button></div>
            </div>
        </div>}
    </div>;
}
