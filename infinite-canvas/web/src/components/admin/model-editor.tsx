...
export function ModelEditor({ model, onSave, onSaved, onRefresh }: Props) {
    ...
    return <form className="min-w-0 rounded-xl border border-[#29385a] bg-[#080b11] p-4 text-[#172033]" onSubmit={(event) => { event.preventDefault(); void submit(); }}>
        <h2 className="text-lg font-semibold">{model ? "编辑逻辑模型" : "新建逻辑模型"}</h2>
        <div className="mt-4 grid min-w-0 gap-3 sm:grid-cols-2">
            <label className="min-w-0 text-sm">模型 ID<input aria-label="模型 ID" disabled={Boolean(model)} value={form.model_id} onChange={(event) => setForm({ ...form, model_id: event.target.value })} className="mt-1 block w-full min-w-0 rounded border border-[#2c3750] bg-[#0c0f17] px-3 py-2 disabled:text-[#848a98]" /></label>
            <label className="min-w-0 text-sm">模型显示名<input aria-label="模型显示名" value={form.display_name} onChange={(event) => setForm({ ...form, display_name: event.target.value })} className="mt-1 block w-full min-w-0 rounded border border-[#2c3750] bg-[#0c0f17] px-3 py-2" /></label>
            <label className="min-w-0 text-sm sm:col-span-2">模型介绍<textarea aria-label="模型介绍" value={form.introduction} onChange={(event) => setForm({ ...form, introduction: event.target.value })} className="mt-1 block min-h-20 w-full min-w-0 rounded border border-[#2c3750] bg-[#0c0f17] px-3 py-2" /></label>
            <label className="min-w-0 text-sm">能力模板<select aria-label="能力模板" disabled={Boolean(model)} value={form.capability_id} onChange={(event) => { const capability = event.target.value as CapabilityId; const first = ADMIN_MODEL_TEMPLATES.find((item) => item.capability === capability); if (first) setForm({ ...form, capability_id: capability, template_id: first.id }); }} className="mt-1 block w-full min-w-0 rounded border border-[#2c3750] bg-[#0c0f17] px-3 py-2">{CAPABILITY_TEMPLATES.map((item) => <option key={item.id} value={item.id}>{item.label}</option>)}</select></label>
            <label className="min-w-0 text-sm">模型类型<select aria-label="模型类型" disabled={Boolean(model)} value={form.template_id} onChange={(event) => setForm({ ...form, template_id: event.target.value as ModelProfileId })} className="mt-1 block w-full min-w-0 rounded border border-[#2c3750] bg-[#0c0f17] px-3 py-2">{profiles.map((item) => <option key={item.id} value={item.id}>{item.label}</option>)}</select></label>
        </div>
        <p className="mt-3 text-xs text-[#8a93a9]">操作：<span className="font-mono text-[#7c9ce7]">{template.contract.operation}</span>。模型创建后 ID 与能力类别保持稳定。</p>
        <button type="submit" disabled={saving} className="mt-4 rounded bg-[#6285d9] px-4 py-2 text-sm font-semibold text-[#070a10] disabled:opacity-50">{saving ? "保存中…" : "保存模型"}</button>
        {message === "failed" && <p role="alert" className="mt-3 text-sm text-[#92400e]">模型未保存，请检查填写内容。</p>}
        {message === "conflict" && <div role="alert" className="mt-3 flex flex-wrap items-center gap-2 text-sm text-[#92400e]">配置已变化，请重新加载。{onRefresh && <button type="button" onClick={onRefresh} className="underline">重新加载</button>}</div>}
    </form>;
}
