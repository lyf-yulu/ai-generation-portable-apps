...
export function ModelRouteEditor({ model, route, pools, onSave, onSaved, onRefresh }: Props) {
    ...
    return <section className="min-w-0 rounded-xl border border-[#29385a] bg-[#080b11] p-4 text-[#172033]">
        <h2 className="text-lg font-semibold">{route ? "编辑调用线路" : "新建调用线路"}</h2>
        <p className="mt-1 text-xs text-[#8a93a9]">凭据由部署配置管理；这里仅绑定兼容的安全凭据池。</p>
        <form className="mt-4 grid min-w-0 gap-3 sm:grid-cols-2" onSubmit={(event) => { event.preventDefault(); void submit(); }}>
            <label className="min-w-0 text-sm">线路 ID<input aria-label="线路 ID" disabled={Boolean(route)} value={form.route_id} onChange={(event) => setForm({ ...form, route_id: event.target.value })} className="mt-1 block w-full min-w-0 rounded border border-[#2c3750] bg-[#0c0f17] px-3 py-2 disabled:text-[#848a98]" /></label>
            <label className="min-w-0 text-sm">线路模板<select aria-label="线路模板" disabled={Boolean(route)} value={form.template_id} onChange={(event) => { const next = templates.find((item) => item.id === event.target.value) || template; setForm({ ...form, template_id: next.id, pool_id: "" }); }} className="mt-1 block w-full min-w-0 rounded border border-[#2c3750] bg-[#0c0f17] px-3 py-2 disabled:text-[#848a98]">{templates.map((item) => <option key={item.id} value={item.id}>{item.routeLabel}</option>)}</select></label>
            <label className="min-w-0 text-sm">Provider<select aria-label="Provider" value={form.provider_id} onChange={(event) => setForm({ ...form, provider_id: event.target.value, pool_id: "" })} className="mt-1 block w-full min-w-0 rounded border border-[#2c3750] bg-[#0c0f17] px-3 py-2"><option value="">请选择</option>{providers.map((provider) => <option key={provider} value={provider}>{provider}</option>)}</select></label>
            <div className="min-w-0 text-sm">模型族<div aria-label="模型族" className="mt-1 min-h-10 w-full min-w-0 rounded border border-[#2c3750] bg-[#0c0f17] px-3 py-2 font-mono text-[#a0b1d7]">{effectiveFamily}</div></div>
            <label className="min-w-0 text-sm">供应商模型名<input aria-label="供应商模型名" value={form.provider_model_name} onChange={(event) => setForm({ ...form, provider_model_name: event.target.value })} className="mt-1 block w-full min-w-0 rounded border border-[#2c3750] bg-[#0c0f17] px-3 py-2" /></label>
            <label className="min-w-0 text-sm">凭据池<select aria-label="凭据池" value={form.pool_id} onChange={(event) => setForm({ ...form, pool_id: event.target.value })} className="mt-1 block w-full min-w-0 rounded border border-[#2c3750] bg-[#0c0f17] px-3 py-2"><option value="">请选择兼容池</option>{availablePools.map((pool) => <option key={pool.pool_id} value={pool.pool_id}>{pool.pool_id} · {pool.group}</option>)}</select></label>
            <label className="min-w-0 text-sm">优先级<input aria-label="优先级" type="number" min={0} max={1_000_000} value={form.priority} onChange={(event) => setForm({ ...form, priority: Number(event.target.value) })} className="mt-1 block w-full min-w-0 rounded border border-[#2c3750] bg-[#0c0f17] px-3 py-2" /></label>
            <label className="min-w-0 text-sm">最大并发<input aria-label="最大并发" type="number" min={1} max={4096} value={form.max_concurrency} onChange={(event) => setForm({ ...form, max_concurrency: Number(event.target.value) })} className="mt-1 block w-full min-w-0 rounded border border-[#2c3750] bg-[#0c0f17] px-3 py-2" /></label>
            <div className="sm:col-span-2"><button type="submit" disabled={!canSubmit} className="rounded bg-[#6285d9] px-4 py-2 text-sm font-semibold text-[#070a10] disabled:opacity-50">{saving ? "保存中…" : "保存线路"}</button></div>
        </form>
        {!route && compatiblePools.length === 0 && <p role="alert" className="mt-3 text-sm text-[#92400e]">尚未配置兼容的{model.modality === "video" ? "视频" : "图像"}凭据池；请先由部署管理员添加对应 Provider、模型族和凭据池。</p>}
        {!route && compatiblePools.length > 0 && missingFields.length > 0 && <p role="status" className="mt-3 text-sm text-[#8a93a9]">保存前请填写：{missingFields.join("、")}。</p>}
        {unsupportedExisting && <p role="alert" className="mt-3 text-sm text-[#92400e]">该历史线路不属于当前受信模板，已设为只读；请停用后新建兼容线路。</p>}
        {message === "failed" && <p role="alert" className="mt-3 text-sm text-[#92400e]">线路未保存，请检查 Provider、模型族和凭据池是否兼容。</p>}
        {message === "conflict" && <div role="alert" className="mt-3 flex gap-2 text-sm text-[#92400e]">配置已变化，请重新加载。{onRefresh && <button type="button" onClick={onRefresh} className="underline">重新加载</button>}</div>}
        <div className="mt-5 grid min-w-0 gap-2 sm:grid-cols-2" aria-label="安全凭据池状态">{pools.map((pool) => <article key={pool.pool_id} className="min-w-0 rounded-lg border border-[#222e48] bg-[#0b0f17] p-3 text-xs">
            <h3 className="truncate font-medium text-[#99b3f0]">{pool.pool_id}</h3>
            <p className="mt-1 text-[#acb4c6]">{pool.provider_id} · {pool.group} · {pool.key_count} 把凭据</p>
