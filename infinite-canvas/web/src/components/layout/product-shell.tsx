    ...
    return (
        <div className="h-dvh overflow-hidden bg-background text-foreground">
            <header className="flex h-14 items-center justify-between gap-2 overflow-x-auto border-b border-border bg-card px-3 md:hidden">
                ...
                <nav className="flex shrink-0 gap-1">{navigation.map(({ label, to }) => <NavLink key={label} to={to} className="rounded px-2 py-1 text-xs text-muted-foreground">{label}</NavLink>)}</nav>
            </header>
            <aside className="fixed inset-y-0 left-0 z-30 hidden w-56 border-r border-border bg-card p-4 md:flex md:flex-col">
                ...
                <nav className="mt-6 space-y-1" aria-label="主导航">
                    {navigation.map(({ label, to, icon: Icon }) => <NavLink key={label} to={to} className={({ isActive }) => `flex items-center gap-3 rounded-lg border-l-2 px-3 py-2.5 text-sm transition-colors ${isActive ? "border-primary bg-accent text-accent-foreground" : "border-transparent text-muted-foreground hover:bg-muted hover:text-foreground"}`}><Icon className="size-4" />{label}</NavLink>)}
                </nav>
