...
export function AuthGate({ children }: { children: ReactNode }) {
    ...
    if (loading || (!session && !errorCode)) {
        return <main className="flex h-dvh items-center justify-center bg-[#f3f6fa] text-sm text-[#465267]">正在验证登录状态…</main>;
    }
    if (!session && errorCode && !["AUTH_REQUIRED", "unauthorized"].includes(errorCode)) {
        return <main className="flex h-dvh flex-col items-center justify-center gap-3 bg-[#f3f6fa] text-[#172033]"><p>暂时无法验证登录状态</p><button className="rounded border border-[#344365] px-3 py-2 text-sm text-[#7196ed]" onClick={() => useSessionStore.setState({ errorCode: null })}>重新连接</button></main>;
    }
...
export function RoleGate({ allowed, children }: { allowed: Array<"admin" | "user" | "viewer">; children: ReactNode }) {
    ...
    if (!session || !allowed.includes(session.role)) {
        return <main className="flex h-full items-center justify-center bg-[#f3f6fa] text-[#465267]">页面不存在</main>;
    }
    return <>{children}</>;
}
