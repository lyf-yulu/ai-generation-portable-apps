...
export function GenerationNodeCard({ node, onRetry, onDelete }: { node: CanvasNodeData; onRetry?: (token: string) => void; onDelete?: () => void }) {
    ...
    return (
        <article data-testid={result ? `result-node-${result}` : `generation-node-${node.id}`} className="overflow-hidden rounded-xl border border-[#d9e0ea] bg-[#f8fafc] text-xs text-[#172033] shadow-xl">
            ...
            {status === "success" && node.metadata?.content ? (
                <footer className="flex gap-2 border-t border-[#e2e8f0] p-2">
